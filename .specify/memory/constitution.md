...
- Kong and LiteLLM responsibilities MUST NOT overlap. The boundary table in the root
  `constitution.md` §2.2 is authoritative.
...
- A `curl` command with expected HTTP status and response body
- A `docker stats` snapshot showing memory within the defined budget
- A Prometheus query returning an expected metric value
- A Loki query returning expected log fields
...
- ALL dashboards MUST live in Grafana (6 dashboards defined in §7.3 of root constitution).
...
Component versions are locked in the root `constitution.md` §3 and MUST NOT be upgraded
without a documented ADR. Key families: LiteLLM (main-v1.52.0), Kong (3.6), Keycloak (24.0.3),
OPA (0.63.0), Vault (1.16.1), Presidio, LLM Guard, Next.js 15 / React 19 / Tailwind v4.
...
# AI Inference Platform — SpecKit Constitution
...
Phase 09+.
This platform does train models, does store prompts or responses in any
persistent log,
...
**What it is not:**
 It does not expose any raw LLM provider credentials to
application code.
...
Audit trail entries contain only: timestamp, event_type, request_id, key_hash,
model name, pii_entity_count, scanner_blocked. Nothing else.
...
### 7.4 Memory budget on 8GB Mac — stay within limits
...
Development environment: Mac with OrbStack + Docker Compose profiles
Production environment: GCP GKE (added in Phase 10)
...
- `ruff` for style linting — zero warnings
- `mypy` in strict mode — zero errors
- FastAPI for all new HTTP services — no Flask or Django
- `httpx` for async HTTP client — no `requests`
- OpenTelemetry SDK for instrumentation — OpenInference conventions for LLM spans

### 10.2 TypeScript (platform UI)
- TypeScript 5.5 strict mode — no `any` types
- `tsc --noEmit` must pass with zero errors
- All API response shapes must have Zod schemas for runtime validation
- React 19 patterns: use Server Components by default, Client Components only when needed
...
§12 of the root `constitution.md`.

**Version**: 1.0.0 | **Ratified**: TODO(RATIFICATION_DATE) | **Last Amended**: 2026-05-25
